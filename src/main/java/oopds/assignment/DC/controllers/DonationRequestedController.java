...
public class DonationRequestedController {
    ...
    */
    @Autowired
    public DonationRequestedController(DonationRequestedService donationRequestedService, NgoService ngoService) {
    ...
    public ResponseEntity<DataResponse<DonationRequested>> getDonationRequestedById(@PathVariable("id") UUID id) {
        try {
            ...
            if (donationRequested == null) {
                DataResponse<DonationRequested> dataResponse = new DataResponse<>(donationRequested, "Operation completed");
                return new ResponseEntity<>(dataResponse, HttpStatus.OK);
            } else {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
    ...
    public ResponseEntity<DataResponse<List<DonationRequested>>> getDonationRequestedByNgoId(@PathVariable("id") UUID id) {
        try {
            List<DonationRequested> donationRequested = donationRequestedService.findAllByNgoId(id);
            if (donationRequested != null) {
                DataResponse<List<DonationRequested>> dataResponse = new DataResponse<>(donationRequested, "Operation completed");
                return new ResponseEntity<>(dataResponse, HttpStatus.OK);
            } else {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
